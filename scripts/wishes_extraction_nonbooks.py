"""Extract nonbook wishes from messages."""
...
from src.dto.dto_wishes import NonbookWishExtractionResponse
...
class NonbookWishExtractor(BaseWishExtractor[NonbookWishExtractionResponse]):
    """Extractor for nonbook wishes."""
    ...
    def get_response_format(self) -> type[NonbookWishExtractionResponse]:
        return NonbookWishExtractionResponse

    def clean_results(
        self, results: list[NonbookWishExtractionResponse]
    ) -> pd.DataFrame:
        """Clean and deduplicate nonbook wish results."""
        if not results:
            return pd.DataFrame()
        ...
        # Clean fields
        df["object_name"] = df["object_name"].fillna("").str.strip()
        df["category"] = df["category"].fillna("").str.strip()
        df["object_url"] = df["object_url"].fillna("").str.strip()
        ...
        df = df[
            (df["object_name"] != "")
            & (df["confidence"] >= 0.6)
            & (df["category"].isin(self.ALLOWED_CATEGORIES))
        ]
...
async def main_async() -> None:
    ...
    config = ExtractionConfig(
        ...
        n_retries=args.n_retries,
        retry_delay_s=args.retry_delay_s,
        batch_subdir="nonbook_batches",
    )

    extractor = NonbookWishExtractor(config, client)
    ...
    # Filter messages
    nonbook_messages = extractor.filter_messages_by_type(topics_df, messages_df)
    ...
    # Process all batches
    result_df = await extractor.process_all_batches(nonbook_messages, output_path)
